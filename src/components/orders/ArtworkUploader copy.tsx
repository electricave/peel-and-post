...
import { getArtworkFiles, insertArtworkFile, deleteArtworkFile } from '@/lib/queries';
...
export default function ArtworkUploader({
  ...
  useEffect(() => {
    getArtworkFiles(supabase, orderId).then(setFiles).catch(console.error);
  }, [orderId]);
  ...
  async function handleFiles(selected: FileList | null) {
    ...
    for (const file of Array.from(selected)) {
      ...
      try {
        const record = await insertArtworkFile(supabase, {
          order_id: orderId,
          user_id: userId,
          file_name: file.name,
        ...
        setError(`Uploaded but failed to save record: ${dbErr.message}`);
      }
    }
  ...
  async function handleDelete(file: ArtworkFile) {
    ...
    await deleteArtworkFile(supabase, file.id);
